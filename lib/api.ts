...
/**
 * رفع ملف (صورة / مستند / أي نوع) إلى R2 وإرجاع الرابط العام
 */
export async function uploadFile(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          const data = JSON.parse(xhr.responseText);
          if (data.url) resolve(data);
          else reject(new Error(data.error || 'لم يُرجع الخادم رابطاً للملف'));
        } catch {
          reject(new Error('استجابة غير صالحة من الخادم'));
        }
      } else {
    ...
    xhr.onerror = () => reject(new Error('فشل الاتصال بالخادم أثناء رفع الملف'));
