  ...
  return (
    <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none', userSelect: 'none' }}>
      <svg width="38" height="38" viewBox="0 0 54 54" fill="none">
        <defs><radialGradient id="rg_forum" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="#EDF7BD"/><stop offset="42%" stopColor="#85C79A"/><stop offset="100%" stopColor="#4E8D9C"/></radialGradient></defs>
        <circle cx="27" cy="27" r="26" fill="rgba(78,141,156,0.1)"/>
        <path d="M27 7.5 L29.8 18.5 L41.5 20.5 L33 29 L35.5 41 L27 34.5 L18.5 41 L21 29 L12.5 20.5 L24.2 18.5 Z" fill="url(#rg_forum)"/>
...
export default function ForumPage() {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#080520', color: '#fff', fontFamily: "'Cairo', sans-serif", direction: 'rtl' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 100, height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', background: 'rgba(8,5,32,0.96)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(78,141,156,0.2)', boxShadow: '0 2px 32px rgba(0,0,0,0.5)' }}>
        <GalaxyLogo />
        <nav style={{ display: 'flex', gap: 4 }}>
          {[{ href: '/news', label: 'الأخبار' }, { href: '/campaigns', label: 'الحملات' }, { href: '/marketplace', label: 'السوق' }, { href: '/cloud', label: '☁️ SAAS' }, { href: '/services', label: 'الخدمات' }, { href: '/forum', label: 'المنتدى', active: true }].map(link => (
