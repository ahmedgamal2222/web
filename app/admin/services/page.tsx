...
export default function AdminServices() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const params = new URLSearchParams({ page: String(page), limit: String(limit), status: status || 'all' });
    if (category) params.set('category', category);
    const res  = await fetch(`${API_BASE}/api/services?${params}`, { headers });
  ...
  const deleteItem = async (id: number) => {
    ...
    await fetch(`${API_BASE}/api/services/${id}`, { method: 'DELETE', headers }).catch(() => {});
    fetchItems();
  };

  const CAT_LABELS: Record<string, string> = {
    design: 'تصميم', translation: 'ترجمة', writing: 'كتابة',
    media: 'إعلام', marketing: 'تسويق', tech: 'تقنية',
  };

  const STATUS_LABELS: Record<string, string> = {
    active: 'نشط', inactive: 'غير نشط', pending: 'بانتظار المراجعة',
  };
  ...
  return (
    <div className="admin-page">
      <div className="admin-header">
        <div>
          <h1 style={{ fontSize: '1.8rem', marginBottom: 6 }}>🛠️ إدارة الخدمات</h1>
          <p style={{ opacity: 0.7 }}>مراجعة وإدارة جميع الخدمات المقدمة</p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: `${COLORS.darkNavy}08`, borderBottom: `2px solid ${COLORS.teal}20` }}>
                {['#', 'العنوان', 'التصنيف', 'مقدم الخدمة', 'السعر', 'التقييم', 'الحالة', 'إجراءات'].map(h => (
            ...
            <tbody>
              {items.map((item: any, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '12px 16px', fontSize: '0.88rem', color: COLORS.darkNavy }}>
                    {item.provider_name_ar || item.provider_name || '—'}
                  </td>
                  <td style={{ padding: '12px 16px', fontWeight: 700, color: COLORS.darkNavy, fontSize: '0.9rem' }}>
                    {item.price ? `${item.price} ${item.currency || ''}` : 'مجاني'}
                  </td>
                  <td style={{ padding: '12px 16px', fontSize: '0.85rem', color: '#f59e0b' }}>
                    {item.rating ? `⭐ ${Number(item.rating).toFixed(1)} (${item.reviews_count || 0})` : '—'}
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{
                      background: item.status === 'active' ? '#10b98118' : item.status === 'pending' ? '#f59e0b18' : '#ef444418',
